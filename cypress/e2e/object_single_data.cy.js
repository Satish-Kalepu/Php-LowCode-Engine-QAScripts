describe('PHP Engine', () => {
  ...
  function createFamilyTree() {
    ...
    for (let index = 0; index < familyData.length; index++) {
      // cy.log(familyData[index].l)
      let propsData = familyData[index].props;
      let propsArray = Object.entries(propsData);

      propsArray.forEach(([key, value]) => {
        // cy.get('[data-list-filter="T,GT"]').click()
        // cy.get('div.context_item').contains('Object Thing').click()
        // cy.get('[data-var="ref:thingnew:data:thing:l:v"]').click()
        // cy.get('#context_menu__ > .btn').click()
        // cy.get('.code_line > :nth-child(1) > [title="Thing"]').click();
        cy.log(key)
      })
      // cy.get('#contextmenu_key1').type('')
    }
  }
